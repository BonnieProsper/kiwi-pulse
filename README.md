...
This sentiment pipeline returns a score, a 95% credible interval, and a measure of how much to trust it, derived from disagreement between sources, not added as an afterthought.
...
The Normal-Normal conjugate gives a closed-form posterior. No sampling or black box. A high disagreement between sources → high σ² → wider interval. The uncertainty is a direct consequence of the data.
...
Uncertainty is not added after the fact, instead it emerges directly from disagreement in the data.
...
Simulation across 5,000 runs: 99.2% accuracy when acting, 40% abstain rate - versus 92% accuracy with 0% abstain for a baseline that always commits. This tradeoff is explicit and tunable.
...
```
src/
├── core/bayesian_model.py         # inference
...
**Variance floor.** Raw sample variance at n=2 gave 69.6% empirical coverage vs the 95% nominal target. A floor of 0.10 brings this to 92.6%. This is justified by the domain, as LLM scores always carry some irreducible noise.
...
**Lazy client initialisation.** The OpenAI client is initialised on first use, so importing the module doesn't require `OPENAI_API_KEY` as tests run without it.
...
`WRITEUP.md` covers what was tried, what failed, why the truncated Normal didn't help, and what the real-world results actually mean.
